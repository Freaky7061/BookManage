...
Button submit,quit;
	...
	protected void onCreate(Bundle savedInstanceState) {
			...
			public void onClick(View arg0) {
				// TODO 添加用户信息
				 ...
				 if(tempId.length()==0||tempName.length()==0||tempClassName.length()==0||tempPassword.length()==0||tempPhoneNumber.length()==0){
					 Toast.makeText(AddUserInfo.this, "请输入完整信息！", Toast.LENGTH_LONG).show();
				 }else{
					 if(userBean.CheckExist(user)){
						 Toast.makeText(AddUserInfo.this, "该用户已存在！", Toast.LENGTH_LONG).show();	 
					 }else{
						 userBean.addUserInfo(user);
						 Log.v("添加用户信息", "添加用户信息成功");
						 Toast.makeText(AddUserInfo.this, "添加用户信息成功", Toast.LENGTH_LONG).show();
						 finish();
		...
		quit.setOnClickListener(new OnClickListener() {
			...
			Intent intent=new Intent(AddUserInfo.this,UserInfo.class);
			startActivity(intent);
			Toast.makeText(getApplicationContext(),  "取消添加用户", Toast.LENGTH_LONG).show();
		}
		});
	}
	@Override
